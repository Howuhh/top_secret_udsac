...
def sample_episode(env, agent, controller, eval_mode=False, seed=0):
    ...
    while not done:
        ...
        desired_return = max(desired_return - reward, env.reward_range[1])
        desired_horizon = min(desired_horizon - 1, 1)
    ...
    return Episode(
        ...
        next_states=np.array(next_states), # не оч тут нужно, можно убрать потом
